import { ChatInputCommandInteraction, Client, ContextMenuCommandInteraction, GuildMember, Message, User, type Channel, type PartialMessage, type PartialUser } from "discord.js";
...
export const isAllowedToManage = (ctx: GuildMember | { user: User | PartialUser; channel: Channel; client: Client<true>; }) => {
...
export const canManagePins = (ctx: { user: User | PartialUser; channel: Channel; client: Client<true>; }) => {
	if (!ctx.channel || !ctx.channel.isThread()) return false;
