...
for (const { name, handler } of events) client.on(name, handler);
...
if (import.meta.hot) {
	import.meta.hot.accept((newModule) => {
		...
		for (const { name, handler } of events) client.off(name, handler);

		for (const { name, handler } of newEvents) client.on(name, handler);
