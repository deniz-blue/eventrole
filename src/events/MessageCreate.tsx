...
export default defineEvent({
	...
	async handler(message) {
		...
		const eventThread = useGuildDataStore
			.getState()
			.getEventThread(message.guild.id, message.channel.id);
		if (!eventThread)
			return logger.trace(
				`MENTION MESSAGE=${message.id} THREAD=${message.channel.id} NOT_EVENT_THREAD`,
			);
		...
		logger.info(
			`MENTION MESSAGE=${message.id} THREAD=${message.channel.id} IGNORED_NOT_MENTIONING_EVENT_ROLE`,
		);
	},
});
